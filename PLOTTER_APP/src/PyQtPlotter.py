...
class PlotWindow(QWidget):
    ...
    def gen_right_panel(self):
        ...
        # self.tabs.addTab(self.tab3, "Lines")

        # Create first tab
        self.Axes_tab.layout = QFormLayout()
        self.Axes_tab.layout.setHorizontalSpacing(100)
        self.Axes_tab.layout.setVerticalSpacing(10)
        self.Axes_tab.setLayout(self.Axes_tab.layout)
    ...
    def gen_Axes_tab(self):
        ...
        #     self.Axes_tab.layout.addRow(i,QLineEdit())
        #     # self.Axes_tab.layout.addRow(QLabel(''))
        #     # print(getattr(self,i))
        self.LimitsTitle = QLabel('Limits')
        self.LimitsTitle.setStyleSheet('font : 30pt')
        self.Axes_tab.layout.addRow(self.LimitsTitle)
        # Xlimits
        self.LimX = QWidget()
        ...
        self.Axes_tab.layout.addRow(self.LimX)
        ...
        self.Axes_tab.layout.addRow(self.LimY)

        self.ScaleTitle = QLabel('Scales')
        self.ScaleTitle.setStyleSheet('font : 30pt')
        self.Axes_tab.layout.addRow(self.ScaleTitle)
        ...
        self.Axes_tab.layout.addRow(self.checks)

        self.legendTitle = QLabel('Legend')
        self.legendTitle.setStyleSheet('font : 30pt')
        self.Axes_tab.layout.addRow(self.legendTitle)

        lengendloclist = ['None', 'best', 'upper right', 'upper left',
                          'lower left',
                          'lower right', 'right', 'left', 'center left',
                          'center right',
        ...
        self.Axes_tab.layout.addRow(self.legend_loc_menu)

    def gen_Lines_tab(self):
        a = None
    def buttons_right(self):
        """
    ...
    def change_plot_params(self):
        ...
        #     self.data['Ymin'], self.data['Ymax'] = ylimits[0], ylimits[1]
        #     # self.ax.set_ylim(self.data['Ymin'], self.data['Ymax'])
    ...
    def update_data_dict(self):
        ...
        if self.figure.get_axes():
           if not self.xlimits:
               self.xlimits = self.ax.get_xlim()
               self.ylimits = self.ax.get_ylim()

           if self.Xmin.text():
               self.data['Xmin'] = float(self.Xmin.text())
           else:
               self.data['Xmin'] = self.xlimits[0]
           if self.Xmax.text():
               self.data['Xmax'] = float(self.Xmax.text())
           else:
               self.data['Xmax'] = self.xlimits[1]
           if self.Ymin.text():
               self.data['Ymin'] = float(self.Ymin.text())
           else:
               self.data['Ymin'] = self.ylimits[0]
           if self.Xmax.text():
               self.data['Ymax'] = float(self.Ymax.text())
           else:
               self.data['Ymax'] = self.ylimits[1]
